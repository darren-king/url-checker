...
from url_checker.cli import user_cli_input_args, display_check_results
from url_checker.checker import site_is_online


def main():
    """
    Function to run the URL checker
    """

    user_args = user_cli_input_args()
...
def _read_urls_from_file(file):
    ...
    if file_path.is_file():
        with file_path.open() as urls_file:
            urls = [
                url.strip() for url in urls_file
            ]  # strips white space around the URL
            if urls:
                return urls
